...
- **Dependency and CI action updates:** Consolidate the open dependabot updates. Bump `esbuild` 0.28.0 → 0.28.2, `@rcrsr/rill-dev` 0.2.0 → 0.2.2, `@types/node` 26.1.2 → 26.3.0, `knip` 6.29.0 → 6.32.2, `oxfmt` 0.61.0 → 0.65.0, `oxlint` 1.76.0 → 1.80.0, and `vitest` 4.1.10 → 4.1.11; the shared-tooling ranges (`knip`, `oxfmt`, `oxlint`, `vitest`) track the `@rcrsr/rill-dev` 0.2.2 baseline so `STD-DEP-1` stays conformant. Pin `github/codeql-action` `init`/`analyze` to v4.37.6. No CLI runtime changes. ([#72](https://github.com/rcrsr/rill-cli/pull/72))
